	...
	************************************************************************************************************/
	...
	JSDragDropTree.prototype = {
		...
		,
		setMoveDropFunction : function(fn)
		{
			this.moveDrop = fn;
		}
		,
		setReorderDropFunction : function(fn)
		{
			this.reorderDrop = fn;
		}
		,
		...
		expandLevel : function(level)
		{
			var menuItems = document.getElementById(this.idOfTree).getElementsByTagName('LI');
			for(var no=0;no<menuItems.length;no++){
				if ( menuItems[no].getAttribute("level") == level )
					{
					var subItems = menuItems[no].getElementsByTagName('UL');
					if(subItems.length>0 && subItems[0].style.display!='block'){
						JSTreeObj.showHideNode(false,menuItems[no].id);
		...
		*/
		...
		initDrag : function(e)
		{
			...
			if ( JSTreeObj.dragNode_source.getAttribute("newentry") != null )
				return false;
		...
		,
		nodeMoveCompleted: function( newparent, oldparent, nodeid )
		{
			if ( newparent != oldparent )
				{
				if ( JSTreeObj.moveDrop != null )
					JSTreeObj.moveDrop( newparent, oldparent, nodeid, JSTreeObj.subNodesList( 'node' + newparent));
				}
			else
				{
				if ( JSTreeObj.reorderDrop != null )
					JSTreeObj.reorderDrop( newparent, nodeid, JSTreeObj.subNodesList( 'node' + newparent));
				}
		},
		subNodesList: function( node )
		{
			node = $(node);
			...
			if(uls.length>0)
				...
				if(lis.length>0)
					...
					for( var i = 0; i < lis.length; i++ )
						{
						if ( lis[i].level/1 != level )
							continue;
						if ( s == "" )
							s = lis[i].getAttribute( "nodeid" );
						else
							s = s + "-" + lis[i].getAttribute( "nodeid" );
						}
					}
				}
			return s;
		...
		dropDragableNodes:function()
		{
			...
			if(JSTreeObj.dragNode_destination)
				{	// Check depth
//				Object.inspectentries( 
				if ( JSTreeObj.dragNode_destination.getAttribute( 'menutree' ) != JSTreeObj.dragNode_source.getAttribute( 'menutree' ) || JSTreeObj.dragNode_destination.getAttribute( 'newentry' ) != null )
					JSTreeObj.dragNode_destination = false;
				else
					{
			...
			oldparent = JSTreeObj.dragNode_source.getAttribute( 'parentid' );
			nodeid = JSTreeObj.dragNode_source.getAttribute( 'nodeid' );
			if(JSTreeObj.dragNode_destination)
				{			
				if(JSTreeObj.insertAsSub)
					{
					var uls = JSTreeObj.dragNode_destination.getElementsByTagName('UL');
					newparent = JSTreeObj.dragNode_destination.getAttribute( 'nodeid' );
					if(uls.length>0)
						{
						ul = uls[0];
						ul.style.display='block';
					...
					if(JSTreeObj.dragNode_destination.nextSibling)
						...
						newparent = nextSib.getAttribute( 'parentid' );
						}
					else
						{
						var nd = JSTreeObj.dragNode_destination.parentNode.appendChild(JSTreeObj.dragNode_source);
						var part = nd.parentElement;
						if ( part.id != null && 'menutree' == part.id.substring( 0, 'menutree'.length ) )
							newparent = part.getAttribute( 'nodeid' );
						else
							newparent = part.parentElement.getAttribute( 'nodeid' );
						}
					}	
				/* Clear parent object */
			...
			if ( newparent/1 > 0 )
				...
				if ( newparent != oldparent )
					{
					JSTreeObj.dragNode_source.level = 1 + $('node' + newparent).getAttribute( "level" )/1;
					JSTreeObj.dragNode_source.parentid = newparent;
					}
				JSTreeObj.nodeMoveCompleted(newparent, oldparent, nodeid);
				}
		}
		,
		dropNode : function( parent, menu )
		{
			var node=$('node' + menu);
			var tmpObj = node;
		...
		,
		addNode : function( number, parent, menu, title, innertext )
		{
			var newnode = $('node' + parent + 'new' );
			...
			li.nodeid = number;
			li.parentid = parent;
			li.level= newnode.getAttribute('level');
			li.innerHTML = innertext;			
			var ps = newnode.previousSibling;
			var nd = null;
			if ( ps != null )
				{
				nd = ps.insertBefore(li);
				}
			else
				{
				nd = newnode.parentNode.insertBefore(li, newnode);
				}
			var img = document.createElement('IMG');
			img.src = this.imageFolder + this.plusImage;
			img.onclick = JSTreeObj.showHideNode;
			img.style.visibility='hidden';
			var aTag = nd.getElementsByTagName('A')[0];
			aTag.id = 'nodeATag' + nd.id.replace(/[^0-9]/gi,'');
			aTag.onmousedown = JSTreeObj.initDrag;
			aTag.onmousemove = JSTreeObj.moveDragableNodes;
			nd.insertBefore(img,aTag);
			var folderImg = document.createElement('IMG');
			folderImg.onmousedown = JSTreeObj.initDrag;
			folderImg.onmousemove = JSTreeObj.moveDragableNodes;
			if(nd.className){
				folderImg.src = this.imageFolder + nd.className;
			}else{
				folderImg.src = this.imageFolder + this.folderImage;
			}
			nd.insertBefore(folderImg,aTag);
			return JSTreeObj.subNodesList( 'node' + parent);
		}
		,
