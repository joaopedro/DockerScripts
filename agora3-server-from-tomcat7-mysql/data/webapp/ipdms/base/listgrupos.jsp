...
<fmt:message key='ipdms.grupo.escolha' var='grupoEscolha'/>
<fmt:message key='ipdms.grupo.gestao' var='grupoGestao'/>
<fmt:message key='ipdms.grupo.designacaosingular' var='grupoDesSingular'/>
<fmt:message key='ipdms.grupo.descricao' var='grupoDescricao'/>
...
<fmt:message key='ipdms.tramitacao.distribuicao.darconhecimento' var='darconhecimentoDes'/>
...
///////////////////NOMES  DAS COLUNAS E GRID ///////////////
		...
		var mainStatus= "${grupoEscolha}";

		//Titulo do gridPanel 
		var titledisp='${grupoGestao}';

		//Titulos das colunas(1)
		var headerCol1='${grupoDesSingular}';
		var headerCol2='${grupoDescricao}';
...
treeLoader = new Ext.tree.TreeLoader({
	url: '${pageContext.request.contextPath}/ajaxListGruposNode.do2',
	<c:if test="${not empty multiple}">
			...
			columns     : [{
		                    	...
		                    	renderer  : function (value, metadata, record){
		                    		return String.format ('<div class="col-move-top" title="${expandSelectNodes}" style="z-index:10000;width:16px; height:16px;top:auto;visibility:visible;" id="{1}" ' +
           							' onclick="expandChildren({1})" alt="${expandSelectNodes}"></div>', record, record.id);
								}           						                                 
		                   }
		                   <c:if test="${darConhecimento}">
                            ,{
                              	id		: "col_darconhec",	
                              	header  : '${darconhecimentoDes}',
                              	width   : 98
                             }
                           </c:if>
		                  ],
