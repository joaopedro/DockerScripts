...
<fmt:message key="ipdms.colaborador.designacaosingular" var="colaboradorDes" />
<fmt:message key="ipdms.notification.encarregar.escolherColaborador" var="escolherColaborador" />
...
var rules = {
	'#backButton' : function(el) {
		el.onclick = function() {
			el.form.action = 'multipleNotification.do2?nids='+$F('nids')+'&referer=${urlanterior}';
...
<html:errors/>
<html:form action="/multipledelegarencarregar">
	<html:hidden property="method" value="${funcao}" />
	<html:hidden property="nids" styleId="nids"/>
	<html:hidden property="page" value="1" />
	<html:hidden property="clear" value="no" />
	<input type="hidden" name="soIds" value="${soIds}"/>
	<input type="hidden" name="urlanterior" value="urlanterior" />
	
	<logic:iterate id="attr" name="multipleDelegacaoForm" property="respondAttribute">
	...
	<FIELDSET>
		<LEGEND>
			<a href="#"	onclick="showOrganicaColaborador();">${escolherColaborador}</a>
		</LEGEND> 
		<html:hidden property="colaborador" styleId="idColaboradores" />
