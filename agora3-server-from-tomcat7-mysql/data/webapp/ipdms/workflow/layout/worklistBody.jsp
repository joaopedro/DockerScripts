...
<fmt:message key="ipdms.forms.anterior" var="formAnterior" />
<fmt:message key="ipdms.forms.seguinte" var="formSeguinte" />
<fmt:message key="ipdms.etapa.designacaosingular" var="etapaDesignacaosingular" />
...
<fmt:message key="ipdms.etapa.emptyProcess" var="emptyProcess" />

<script type="text/javascript">
	var etapaId = <bean:write name="<%= com.sinfic.ipdms.tramitacao.action.BaseWorkflowAction.CURRENT_ETAPA %>" property="numeroConteudo"/>;
	var referer = '<%= org.apache.struts.util.RequestUtils.encodeURL("/worklist.do2?" + request.getQueryString()) %>';
...
<script type="text/javascript">	
	document.title = '${productName} - <logic:present name="anterior">
		[ <a href="<bean:write name="anterior"/>" style="color: white;" title="<bean:write name="titulo_anterior"/>">${formAnterior}</a> ]
			</logic:present>
				${etapaDesignacaosingular}: <bean:write name="etapa" property="titulo"/> 
			<logic:present name="seguinte">
				[ <a href="<bean:write name="seguinte"/>" style="color: white;" title="<bean:write name="titulo_seguinte"/>">${formSeguinte}</a> ]
			</logic:present>';	 
</script>
<div class="row" style="text-align:left; padding-top:0px; padding-bottom:0px;">
	<table style="border:none;" cellpadding="0" cellspacing="0" class="headings">
		<tbody>
		  <tr>
		    <td class="titulo1">
		    	<logic:present name="anterior">
					[ <a href="<bean:write name="anterior"/>" style="color: white;" title="<bean:write name="titulo_anterior"/>">${formAnterior}</a> ]
				</logic:present>
					${etapaDesignacaosingular}: <bean:write name="etapa" property="titulo"/>&nbsp;&nbsp;
				<logic:present name="seguinte">
...
<logic:greaterThan name="numberOfItens" value="0">
	<display:table id="notification" name="worklist" htmlId="worklist" pagesize="20" requestURI="/worklist.do2" style="cursor: pointer">
