...
<div id="ajuda" class="info" style="padding-top:0px; display: none; text-align:left;">
	<table style="border:none" cellpadding="0" cellspacing="0" class="headings">
		<tbody>
		  <tr>
		    <td class="titulo1"><fmt:message key="ipdms.gestao.de"/>&nbsp;<fmt:message key="ipdms.${bundleName}.designacaoplural"/></td>
		    <td style="width:3%; vertical-align:top; text-align:center;"><img src="${pageContext.request.contextPath}/images/icon/icon-ajuda.png" onclick="Javascript:verAjuda('n');" class="cursorMao" alt="<fmt:message key='ipdms.forms.ocultarAjuda'/>" title="<fmt:message key='ipdms.forms.ocultarAjuda'/>" /></td>
		  </tr>
		</tbody>
	</table>		
	<fmt:message key="ipdms.etapa.list.help"/>
</div>
	...
	<c:choose>
	  <c:when test="${not empty listFormBean.elementos}">
	    ...
	    <logic:iterate name="listFormBean" property="elementos" id="elementos" indexId="counter">
		  ...
		  <tr>
		  	<td class="centro">
		  		<nested:link action="/edit${actionName}" paramId="numero" paramProperty="numeroConteudo">
			  			${elementos.titulo }
			  	</nested:link>
		  	</td>
		  	<td class="alignCenter">
		  		<label for="eliminarLabel${counter}"><nested:checkbox property="seleccionado" indexed="true" styleClass="radion" styleId="eliminarLabel${counter}" style="border:none;"/></label>
	  ...
	  <c:otherwise>
	  	<tr>
	  		<td colspan="2" class="alignCenter"><fmt:message key='ipdms.etapa.empty.msg'/></td>
	  	</tr>
