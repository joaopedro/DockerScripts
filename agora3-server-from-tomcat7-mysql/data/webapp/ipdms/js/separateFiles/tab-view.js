	...
	************************************************************************************************************/		
	...
	var IMAGE_DIR = 'ipdms/js/separateFiles/images/';
	...
	function showTab(parentId,tabIndex,reload)
	{
		...
		if(activeTabIndex[parentId]>=0){
			if(activeTabIndex[parentId]==tabIndex && !reload){
				return;
			}
			...
			img.src = IMAGE_DIR + 'tab_right_inactive.gif';
		...
		img.src = IMAGE_DIR + 'tab_right_active.gif';
		...
		var divs = document.getElementById('tabView' + parentId_div +  tabIndex).getElementsByTagName('DIV');
      	for(var no=0;no<divs.length;no++){
			if(divs[no].className=='dynamicContent'){
				if(dynamicContent_urlArray[divs[no].id] != undefined){
					ajax_loadContent_tab(divs[no].id,dynamicContent_urlArray[divs[no].id]);
				}
         	}
		}
	...
	function rolloverTab()
	{
		if(this.className.indexOf('tabInactive')>=0){
			...
			img.src = IMAGE_DIR + 'tab_right_over.gif';
		}
	...
	function initTabs(mainContainerID,tabTitles,activeTab,width,height,closeButtonArray,additionalTab)
	{
		...
		for(var no=0;no<tabTitles.length;no++){
			...
			span.id = 'spanTab' + mainContainerID + "_" + (no + tabView_countTabs[mainContainerID])
			...
			if(closeButtonArray[no]){
				...
				closeButton.src = IMAGE_DIR + 'close.gif';
