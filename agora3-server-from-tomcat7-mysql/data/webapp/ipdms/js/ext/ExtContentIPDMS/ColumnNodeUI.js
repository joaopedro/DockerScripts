...
Ext.ux.tree.ColumnNodeUI = Ext.extend(Ext.tree.TreeNodeUI, {
    ...
    /*onCheckChange : function(){ 
    	...
    	var checked=!(this.node.attributes.checkbox.checked);       
        this.node.attributes.checkbox.checked = checked;
        Ext.getDom('check_'+this.node.attributes.id).checked=checked;
       ...
       //change the "var buf =" line to create the kind of markup that we want to see in your node. 
        var buf = [
         ...
         for(var i = 1, len = cols.length; i < len; i++){
             ...
             var idExist = typeof c.id != 'undefined'; 
             if(idExist && c.id.indexOf("col_darconhec") != -1){            	 
            	buf.push(cb ? ('<input class="x-tree-node-cb" type="checkbox"' + (a.checkbox_dconhe.name ? ' name="' + a.checkbox_dconhe.name + '"' : "name=asd ") + 
            				(a.checkbox_dconhe.value ? ' value="' + a.checkbox_dconhe.value + '"' : "") + (a.checkbox_dconhe.id ? ' id="' + a.checkbox_dconhe.id + '"' : "") + 
            				(a.checkbox_dconhe.checked ? ' checked="checked"' : "") + (a.checkbox_dconhe.disabled ? 'disabled' : "") + ' />') : '');
             } else {
            	 buf.push(cb ? ('<input class="x-tree-node-cb" type="checkbox"' + (a.checkbox.name ? ' name="' + a.checkbox.name + '"' : "name=asd ") + 
                         	(a.checkbox.value ? ' value="' + a.checkbox.value + '"' : "") + (a.checkbox.id ? ' id="' + a.checkbox.id + '"' : "") + 
                         	(a.checkbox.checked ? ' checked="checked"' : "") + (a.checkbox.disabled ? 'disabled' : "") + ' />') : '');                       
             }
