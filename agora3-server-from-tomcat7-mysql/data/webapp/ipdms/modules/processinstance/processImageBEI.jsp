...
<link rel=stylesheet type=text/css href='${THEMES_DIR}/jbpm/workflowimage.css' />
...
<logic:present name="nodes">
    ...
    <logic:iterate id="node" name="nodes">
       	...
       	<c:set var="x_coord" value="-${poolX + 31}"/>
       	<c:set var="y_coord" value="-${poolY + 4}" />
		<c:set var="x" value="${node.x - x_coord}" />
		<c:set var="y" value="${node.y - y_coord}" />
			
		<c:set var="style" value="top:${y - 14}px;left:${x}px;width:${node.width - 3}px;"/>
		...
		<c:set var="style" value="cursor: pointer; top:${y + 2}px;left:${x + 1}px;width:${node.width - 6}px;height:${node.height - 6}px;"/>
