...
<c:choose>
	...
	<c:otherwise>
		<ajax:displayTag id="displayTagFrameConteudo" ajaxFlag="true" baseUrl="${pageContext.request.contextPath}/ecm/listConteudos.do2">
			<display:table style="cursor: pointer;" id="conteudo" name="${paginatedList}" htmlId="worklist" requestURI="">
				<display:column title="${descricao}" property="descricaoConteudo" class="realce" sortable="true" />				
				<display:column title="${campos}" style="width:6%; text-align:center;">
					<a href="javascript:conteudosDetail('conteudoDetail_${conteudo_rowNum}');">
					...
					<%@ include file="conteudoDetail.jsp" %>					
				</display:column>
			</display:table>			
		</ajax:displayTag>		
	</c:otherwise>
...
<form action="${pageContext.request.contextPath}/ecm/estruturas.do2">
