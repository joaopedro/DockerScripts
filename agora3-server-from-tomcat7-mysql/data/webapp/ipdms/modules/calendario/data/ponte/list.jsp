...
<fmt:message key="calendario.removerPonte" var="removerPonte" bundle="${calendarioBundle}" />
<fmt:message key="data.ponte.delete.confirm" var="deleteConfirm" bundle="${calendarioBundle}" />
...
<c:choose>
	...
	<c:otherwise>
		<html:form styleId="ponteDeleteForm" action="ponte/delete">
			...
			<display:footer>
				...
				<td class="tableFooter alignCenter">
					<img src="${pageContext.request.contextPath}/images/icon/icon-lixo.png"
						onclick="cancelar(event,'${removerPonte}','${deleteConfirm}',deleteFeriadoPonte,'${sim}','${nao}');" 
						class="cursorMao" alt="${removerPonte}" title="${removerPonte}" />
				</td>
			</display:footer>
		</display:table>
		</ajax:displayTag>
