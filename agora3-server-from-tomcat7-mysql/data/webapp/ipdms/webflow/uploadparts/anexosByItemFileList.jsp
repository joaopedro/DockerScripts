...
<c:forEach items="${formUploadListItens}" var="itemWithFile" varStatus="i">
	...
	<c:if test="${!visivel}">			     
		...
		<td class="alignCenter">
						...
						<input type="checkbox" name="checkbox_delete_${i.index}" id="checkbox_delete_${i.index}" style="border:0" /></td>
	</tr>
</c:forEach>
</table>
...
<!-- Icon eliminar -->
<div id="alignRight">
		<img src="${pageContext.request.contextPath}/images/icon/icon-lixo.png" class="cursorMao" alt="${eliminar}" title="${eliminar}" 
			onclick="cancelar(event,'${apagarFicheiros}', '${apagarFicheirosConfirm}', deleteUploadedFiles, '${sim}', '${nao}');" />
</div>