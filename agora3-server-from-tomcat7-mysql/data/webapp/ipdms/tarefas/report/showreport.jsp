...
<fmt:message key="ipdms.relatorio.field.required.msgPt1" var="requiredMsgPt1" />
<fmt:message key="ipdms.relatorio.field.required.msgPt2" var="requiredMsgPt2" />
    ...
    Ext.each(params,function(item,index,params){
        for(id in isreqlist){
            ...
            if(item.name == isreqlist[id] && item.value.length == 0){
                alert("${requiredMsgPt1} " + item.name + " ${requiredMsgPt2}");
                scope.control++;
                return false;
            }
        }                           
...
<form name="formulario" method="post">
	...
	<jsp:include page="../../home/birt/reportparams.jsp" />
	...
	<%--Buttons --%>	
	<div class="buttonRow">
		<input type="button" class="formButton" value="${anterior}" onclick="this.form.action='${pageContext.request.contextPath}${urlAnterior}'; this.form.target='_self'; this.form.method.value='prepare'; this.form.submit();" title="${anteriorTitle}" />
