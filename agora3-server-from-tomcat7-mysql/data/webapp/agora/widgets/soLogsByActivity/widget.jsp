...
<script>
    loadPage = function(nid){

        $('#soLogByActivityTable > tbody tr').remove();
        $.getJSON( "${pageContext.request.contextPath}/agora/getLogEntriesBySO.do?nid="+nid).done(function(json){

            if(json.length==0){
                var tr = '<tr class="no_read"><td colspan="6" style="text-align:center">Não existem eventos</td></tr>';
                $('#soLogByActivityTable > tbody:last').append(tr);
            }else{
